...
// If this widget already exists globally, import it and remove this copy.
class AcademicBuildingsCard extends StatelessWidget {
  ...
  Widget _buildIcon() {
    if (icon is IconData) {
      ...
      return const Icon(Icons.business_center, size: 24, color: Colors.black87);
    }
  }
}

/// New screen: iiim_programs_screen.dart
class IiimProgramsScreen extends StatelessWidget {
  const IiimProgramsScreen({Key? key}) : super(key: key);
  ...
  List<_Item> _pg() => const [
    _Item(
      ...
      icon: 'assets/iiim/mba.png',
    ),
    _Item(
      shortName: 'M.Sc. (Mgmt)',
      fullName: 'Master of Science (Management)',
      icon: 'assets/iiim/msc_mgmt.png',
    ),
  ];
